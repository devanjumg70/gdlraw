...
async fn test_redirect_persists_proxy() {
    ...
    tokio::spawn(async move {
        loop {
            if let Ok((mut client_sock, _)) = proxy_listener.accept().await {
                 ...
                 tokio::spawn(async move {
                     if let Ok(mut server_sock) = tokio::net::TcpStream::connect(format!("127.0.0.1:{}", server_port)).await {
    ...
    assert_eq!(count, 2, "Proxy should be used 2 times (Start + Redirect). Got {}", count);
}
...
async fn test_redirect_strips_auth_cross_origin() {
    ...
    assert_eq!(resp.status(), 200, "Should be 200 OK (Safe). If 403, Auth leaked.");
}
...
async fn test_redirect_persists_headers_same_origin() {
    ...
    assert_eq!(resp.status(), 200, "Custom header should persist on same-origin redirect");
}
